...
public class PictureV3 extends SimplePicture {
  ...
  public String toString() {
    String output = "PictureV3, filename " + getFileName() + " height " + getHeight() + " width " + getWidth();
    return output;
  ...
  public void grayscale() {
    ...
    for (Pixel[] rowArray : pixels) {
      for (Pixel pixelObj : rowArray) {
        int set = (pixelObj.getBlue() + pixelObj.getGreen() + pixelObj.getRed()) / 3;
        pixelObj.setBlue(set);
        pixelObj.setRed(set);
        pixelObj.setGreen(set);
      }
    }
  }
  // grayscale
   ...
   */
  public void copy(PictureV3 fromPic, int fromStartRow, int fromStartCol, int fromEndRow, int fromEndCol, int toStartRow, int toStartCol) {
    Pixel fromPixel;
    Pixel toPixel;
    Pixel[][] toPixels = this.getPixels2D();
    Pixel[][] fromPixels = fromPic.getPixels2D();
  ...
  public void testCopy() {
    PictureV3 flower1 = new PictureV3("flower1.jpg");
    this.copy(flower1, 10, 30, 95, 90, 100, 100);
  }

  /** Method to create a collage of several pictures */
  public void createCollage() {
    PictureV3 flower1 = new PictureV3("flower1.jpg");
    PictureV3 flower2 = new PictureV3("flower2.jpg");
    this.copy(flower1, 0, 0);
    this.copy(flower2, 100, 0);
    this.copy(flower1, 200, 0);
    PictureV3 flowerNoBlue = new PictureV3(flower2);
  ...
  public void myCollage() {
    ...
    temple.keepOnlyBlue();
    seagull.keepOnlyRed();
    this.copy(beach, 100, 100, 250, 250, 0, 0);
    this.copy(temple, 100, 100, 250, 250, 150, 0);
    this.copy(seagull, 100, 100, 250, 250, 300, 0);
    this.mirrorVertical();
  ...
  public void edgeDetection2(int edgeDist) {
    ...
    for (int row = 0; row < pixels.length - 1; row++) {
      for (int col = 0; col < pixels[0].length - 1; col++) {
        ...
        if (thePixel.colorDistance(rightPixel.getColor()) > edgeDist || thePixel.colorDistance(bottomPixel.getColor()) > edgeDist)
          thePixel.setColor(Color.BLACK);
        else
          thePixel.setColor(Color.WHITE);
      }
    }
  }
