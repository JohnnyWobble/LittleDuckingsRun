...
public class ElevensBoard extends Board {
    ...
    public boolean anotherPlayIsPossible() {
        return anotherPlayIsPossible(cardIndexes());
    }
     ...
     *         otherwise.
     */
    private boolean anotherPlayIsPossible(List<Integer> selectedCards) {
    ...
    private boolean containsPairSum11(List<Integer> selectedCards) {
        for (int i : selectedCards) {
            for (int j : selectedCards) {
                if (i == j) {
                    continue;
                } else if (cardAt(i).pointValue() + cardAt(j).pointValue() == 11) {
                    return true;
                }
            }
        }
